...
# convert DICE SE to count matrix + metadata csv, and normalize test datasets (lamprecht, pbmc_10k)
## Parameters ## 
...
df_LABEL['truth'] = df_LABEL['truth'].replace('Monocytes',2)
df_LABEL['truth'] = df_LABEL['truth'].replace('B cells',4)
df_LABEL['truth'] = df_LABEL['truth'].replace('NK cells',5)
## for PBMC 10k ##
#df_LABEL['truth'] = df_LABEL['truth'].replace('T cells, CD4+',8)
#df_LABEL['truth'] = df_LABEL['truth'].replace('T cells, CD8+',6)
## for Lamprecht ##
df_LABEL['truth'] = df_LABEL['truth'].replace('T cells, CD8+',6)
df_LABEL['truth'] = df_LABEL['truth'].replace('T cells, CD4+',6)
...
pbmc_ge_raw = pd.read_csv("Lamprecht_full_normalized_filt.csv", index_col = 0)
pbmc_label = pd.read_csv("Lamprecht_full_celltype_clean.csv", index_col = 0)
pbmc_ge_raw = pbmc_ge_raw[~pbmc_ge_raw.index.duplicated(keep='first')]
pbmc_ge_raw = pbmc_ge_raw[pbmc_ge_raw.index.isin(df_GE.index)]
df_GE = df_GE[df_GE.index.isin(pbmc_ge_raw.index)]
pbmc_ge_raw = pbmc_ge_raw.reindex(df_GE.index)
...
geneset = list(set(pbmc_ge_raw.index))
geneset_clean = [x for x in geneset if not x.startswith('MT-') and not x.startswith('#')]
geneset_clean = [x for x in geneset_clean if not x.startswith('RPL') and not x.startswith('#')]
geneset_clean = [x for x in geneset_clean if not x.startswith('RPS') and not x.startswith('#')]
pbmc_ge = ge_transform(pbmc_ge_raw, genes=geneset_clean)
labels_test_global = pbmc_label
GEs_test_global = pbmc_ge
...
for sub_itr in range(num_exp):
  ...
  pd.DataFrame(set_score_combined).to_csv('dice_lamp_genelist_weights.csv', mode='a', header=False)
...
output_set = pd.read_csv('dice_lamp_genelist_weights.csv', header = None)
genes = pd.DataFrame(output_set.groupby(2).agg({1:'count',3:'mean'}))
genes = genes[genes[1] >= num_exp]
pd.DataFrame(genes).to_csv('dice_lamp_genelist.csv', header = False)
