...
lamprecht_ge_raw = pd.read_csv("Lamprecht_full_normalized_filt.csv", index_col = 0)
lamprecht_label = pd.read_csv("Lamprecht_full_celltype_clean.csv", index_col = 0)
...
for sub_itr in range(num_exp):
  ...
  pd.DataFrame(set_score_combined).to_csv('lampfull_genelist_weights.csv', mode='a', header=False)
...
output_set = pd.read_csv('lampfull_genelist_weights.csv', header = None)
genes = pd.DataFrame(output_set.groupby(2).agg({1:'count',3:'mean'}))
genes = genes[genes[1] >= num_exp]
pd.DataFrame(genes).to_csv('lampfull_genelist.csv', header = False)
