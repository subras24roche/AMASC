...
g <- read_csv('lampfull_genelist.csv', col_names = FALSE)
genes <- as.character(g$X1)

ge <- read.csv('Lamprecht_full_normalized_filt.csv')
ge <- ge[!duplicated(ge$X),]
rownames(ge) <- ge$X
ge <- ge[,-1]
label_gex <- read.csv('Lamprecht_full_celltype_clean.csv', row.names = 1)
...
png("xgboost_progressive_selection_lamp.png",width=1000,height=600,unit="px")
